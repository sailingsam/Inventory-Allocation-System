...
4. Seed the worked-example data **once** (Render dashboard → `oms-backend` → **Shell**):
   ```bash
   python manage.py seed_demo --fresh
   ```
...
- The build step runs `collectstatic` (WhiteNoise serves static assets) and `migrate`.
...
- **Don't** put `seed_demo` in the build/start command — it would wipe data on every deploy.
